...
export interface CopilotAdoption {
   ...
   * one AI-assisted commit counts). Measured over `totalMergedPRs` (the
   * collected history window, roughly the last ~13 months / 1,000 most
   ...
   * `totalDetailedPRs`, a much smaller sample (up to 10 most recent merged
   * PRs per repo) than the merged-PR timeline used for authorship.
   */
 ...
 * ~13-month collected-history window used for PR authorship.
 */
export interface CopilotAgentMetrics {
   ...
   * that one covers ~13 months of collected history and matches on commit/PR
