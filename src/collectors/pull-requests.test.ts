...
describe("parseAIBodySignature", () => {
  it("returns null for null/undefined body", () => {
    expect(parseAIBodySignature(null)).toBeNull();
    expect(parseAIBodySignature(undefined)).toBeNull();
  });

  it("returns null for a plain human PR body", () => {
    expect(parseAIBodySignature("This PR fixes a bug in the parser.")).toBeNull();
  });

  it("detects Co-authored-by trailer in body (copilot)", () => {
    expect(parseAIBodySignature(
      "Fixes #1\n\nCo-authored-by: copilot-swe-agent[bot] <198982749+Copilot@users.noreply.github.com>"
    )).toBe("copilot");
  });

  it("detects Co-authored-by trailer in body (claude)", () => {
    expect(parseAIBodySignature(
      "Some description\n\nCo-authored-by: claude[bot] <claude[bot]@noreply>"
    )).toBe("claude");
  });

  it("detects Co-authored-by trailer in body (codex)", () => {
    expect(parseAIBodySignature(
      "Change\n\nCo-authored-by: codex[bot] <codex[bot]@noreply>"
    )).toBe("codex");
  });

  it("detects 'suggested by copilot' prose signature", () => {
    expect(parseAIBodySignature("_Suggested by Copilot_")).toBe("copilot");
  });

  it("detects 'generated by copilot' prose signature", () => {
    expect(parseAIBodySignature("Generated by Copilot.")).toBe("copilot");
  });

  it("detects 'copilot workspace' prose signature", () => {
    expect(parseAIBodySignature("Created with Copilot Workspace")).toBe("copilot");
  });

  it("detects 'created by github copilot' prose signature", () => {
    expect(parseAIBodySignature("Created by GitHub Copilot")).toBe("copilot");
  });

  it("detects 'generated by claude' prose signature", () => {
    expect(parseAIBodySignature("Generated by Claude")).toBe("claude");
  });

  it("detects 'generated by codex' prose signature", () => {
    expect(parseAIBodySignature("Generated by Codex")).toBe("codex");
  });

  it("does not false-positive on a human email containing 'copilot' as plain text", () => {
    // Body contains word but not as a known prose signature or trailer
    expect(parseAIBodySignature("I used Copilot suggestions to write this.")).toBeNull();
  });

  it("is case-insensitive for prose patterns", () => {
    expect(parseAIBodySignature("SUGGESTED BY COPILOT")).toBe("copilot");
  });
});
...
describe("buildMergedPRTimeline", () => {
  ...
  it("detects AI via individual commit author login (non-squash merge gap)", () => {
    const node = makePRNode({
      author: { login: "alice", __typename: "User" },
      mergeCommit: null, // no squash message to check
      commits: {
        totalCount: 1,
        nodes: [{
          commit: {
            authors: {
              nodes: [{ email: "alice@example.com", user: { login: "alice", __typename: "User" } },
                      { email: "copilot[bot]@users.noreply.github.com", user: { login: "copilot[bot]", __typename: "Bot" } }],
            },
          },
        }],
      },
  ...
  it("detects claude via commit author email on noreply domain", () => {
    const node = makePRNode({
      ...
      commits: {
        ...
        nodes: [{
          commit: {
            authors: {
              nodes: [{ email: "123+claude@users.noreply.github.com", user: null }],
            },
          },
        }],
      },
  ...
  it("does not false-positive on human email containing 'copilot' substring outside noreply domain", () => {
    const node = makePRNode({
      ...
      commits: {
        ...
        nodes: [{
          commit: {
            authors: {
              nodes: [{ email: "copilot-fan@company.com", user: null }],
            },
          },
        }],
      },
  ...
  it("detects AI via PR body signature when merge commit and commit authors are human", () => {
    const node = makePRNode({
      author: { login: "alice", __typename: "User" },
      mergeCommit: { message: "Merge pull request #1 from alice/feature" },
      body: "_Suggested by Copilot_",
    });
    const result = buildMergedPRTimeline([node]);
    expect(result[0].isCopilotAuthored).toBe(true);
    expect(result[0].aiAuthorType).toBe("copilot");
  ...
  it("merge commit with no AI trailer does not block commit-author or body checks", () => {
    // Previously resolveAIType returned null immediately when merge commit existed
    // but had no AI trailer, skipping commit authors and body entirely.
    const node = makePRNode({
      author: { login: "alice", __typename: "User" },
      mergeCommit: { message: "Merge pull request from alice (no AI trailer)" },
  ...
  it("PR author detection wins over commit author and body signals", () => {
    const node = makePRNode({
      author: { login: "copilot[bot]", __typename: "Bot" },
      mergeCommit: null,
      body: "Generated by Claude",
      commits: {
        totalCount: 1,
        nodes: [{ commit: { authors: { nodes: [{ email: "123+claude@users.noreply.github.com", user: null }] } } }],
      },
    });
    const result = buildMergedPRTimeline([node]);
    expect(result[0].aiAuthorType).toBe("copilot");
